...
#5 years
f1 <- cph(Surv(futime, fustat) ~ risk + N + Chemotherapy, x=T, y=T, surv=T,time.inc=5, data = rt)
cal1 <- calibrate(f1,cmethod="KM",u=4,method="boot",legend=FALSE,rule="aic",m=20,b=1000)
plot(cal1,
     lwd=3,
     lty=1,
...
#2 years
f1 <- cph(Surv(futime, fustat) ~ risk + N + Chemotherapy, x=T, y=T, surv=T,time.inc=2, data = rt)
cal1 <- calibrate(f1,cmethod="KM",u=3,method="boot",legend=FALSE,rule="aic",m=20,b=1000)
plot(cal1,
     lwd=3,
     lty=1,
     xlab="Predicted Probability",
