...
model1 <- decision_curve(fustat ~ risk + N + Chemotherapy,
                         data = rt,
                         thresholds = seq(0, 1, by = .005),
                         bootstraps = 10)
model2<- decision_curve(fustat ~  risk ,
                       data = rt,
                       thresholds = seq(0, 1, by = .005),
                        bootstraps = 10)
plot
...
graph2ppt(file="5 years.pptx", width=10, height=6)
...
plot_decision_curve(list(model1, model2),confidence.intervals=FALSE,
                    ...
                    standardize=FALSE,
                    legend.position = "none")
graph2ppt(file="2 years.pptx", width=10, height=6)
